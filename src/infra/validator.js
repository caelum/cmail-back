...
class Validator {
  ...
  constructor(request, validations) {
    ...
    for (const validation in validations) {
      this._validations[validation] = new Validation(
        validation,
        validations[validation]
      );

      console.log(this._validations[validation]);
    }
  }
...
export const validatorMiddleware = ({ schemaMetadata }) => (req, res, next) => {
  const validator = new Validator(req, schemaMetadata);
...
const formatJoiErrors = joiError => {
  const errResponseFormated = joiError.details.reduce(
    (errorObj, currentError) => {
